import { LogFunction, LogFunctionFactory, StyleInterface } from '../types';
import { Chalk } from 'chalk';

/**
 * Creates a converter for a HEX color
 * @param color
 * @param baseConverter
 * @param chalkFnName
 */
function getConverter(
	color: string,
	baseConverter: Chalk,
	chalkFnName: 'hex' | 'bgHex'
) {
	if (/^#[0-9a-fA-F]{3,6}$/.test(color)) {
		baseConverter = baseConverter[chalkFnName](color);
	} else if (colors.hasOwnProperty(color.toLowerCase())) {
		baseConverter = baseConverter[chalkFnName](colors[color.toLowerCase()]);
	}
...
export const ChalkLogger: (ch: Chalk) => LogFunction = (ch: Chalk) => {
	...
	function getChalkFunctionForStyle(style: StyleInterface) {
		return getConverter(
			style.background,
			getConverter(style.color, ch.bold, 'hex'),
			'bgHex'
		);
	}
