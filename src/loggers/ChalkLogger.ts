...
/**
 * A {@link LogFunction} to log the the terminal in NodeJS using the {@link https://github.com/chalk/chalk|chalk} library
 *
...
export default function ChalkLogger(ch: Chalk): LogFunction {
    ...
    function getChalkFunctionForStyle(style: StyleInterface) {
        ...
        if (/^#[0-9a-fA-F]{3,6}$/.test(style.color)) {
            fn = fn.hex(style.color);
        } else if (colors.hasOwnProperty(style.color.toLowerCase())) {
            fn = fn.hex(colors[style.color.toLowerCase()]);
        }

        if (/^#[0-9a-fA-F]{3,6}$/.test(style.background)) {
            fn = fn.bgHex(style.background);
        } else if (colors.hasOwnProperty(style.background.toLowerCase())) {
            fn = fn.bgHex(colors[style.background.toLowerCase()]);
        }
    ...
    return (
        ...
        console.log(
            `${typeFn(` ${type} `)}${componentFn(` ${componentName} `)}`,
            ...args
        );
    };
}
